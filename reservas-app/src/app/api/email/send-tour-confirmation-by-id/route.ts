// src/app/api/email/send-tour-confirmation-by-id/route.ts
...
interface SendTourConfirmationByIdRequest {
  tourId: string;
...
export async function POST(request: NextRequest) {
  try {
    const body: SendTourConfirmationByIdRequest = await request.json();

    // Validate required fields
    if (!body.tourId) {
      return NextResponse.json(
        { error: 'Missing required field: tourId' },
        { status: 400 }
      );
    }

    const { tourId, transactionId, authCode } = body;

    console.log('[Tour Email API] Fetching tour booking data by ID:', tourId);
    ...
    if (!customerEmail) {
      console.error('[Tour Email API] No customer email found for tour:', tourId);
      return NextResponse.json(
    ...
    const formattedBookingId = formatBookingId(tourBooking.booking_id);
    ...
    // Build email HTML (same as send-tour-confirmation route)
    const emailHtml = `
<!DOCTYPE html>
<html>
<head>
